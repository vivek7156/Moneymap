    ...
    <div className='flex flex-wrap items-center gap-4'>
      <SkeletonWrapper isLoading={historyPeriods.isFetching} fullWidth={false}>
        <Tabs value={timeframe}  onValueChange={(value) => { setTimeframe(value as Timeframe);}} >
            <TabsList>
                <TabsTrigger value='year' className={timeframe === 'year'?'active':''}>Year</TabsTrigger>
                <TabsTrigger value='month' className={timeframe === 'month'?'active':''}>Month</TabsTrigger>
            </TabsList>
        </Tabs>
      </SkeletonWrapper><div className='flex flex-wrap items-center gap-2'>
        <SkeletonWrapper isLoading={historyPeriods.isFetching} fullWidth={false}>
            <YearSelector period={period} setPeriod={setPeriod} years={historyPeriods.data || []} />
        </SkeletonWrapper>
        {timeframe === "month" && (
            <SkeletonWrapper isLoading={historyPeriods.isFetching} fullWidth={false}>
                <MonthSelector period={period} setPeriod={setPeriod} years={historyPeriods.data || []} />
            </SkeletonWrapper>
        )}
      </div>
    </div>
    ...
    return (
        ...
        onValueChange={(value) => {
            ...
            <SelectTrigger className='w-[180px] h-[30px]'>
                <SelectValue className='font-thin'>{period.year ? period.year : "Select Year"}</SelectValue>
            </SelectTrigger>
            <SelectContent>
                {years.map((year) => (
...
function MonthSelector({period, setPeriod}: {
    ...
    return (
        ...
        onValueChange={(value) => {
            ...
            <SelectTrigger className='w-[180px] h-[30px]'>
            <SelectValue className='font-thin'>{period.month !== undefined
                        ? new Date(period.year, period.month, 1).toLocaleString("default", { month: "long" })
                        : "Select Month"}</SelectValue>
            </SelectTrigger>
            <SelectContent>
